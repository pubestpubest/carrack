    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="mb-6">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            <h1 className={`text-2xl font-bold grade-${itemMeta?.grade ?? 'blue'}`}>
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-900 px-5 py-4">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="text-xl font-bold tabular-nums text-sky-400">{progress}%</span>
          </div>
        ...
        return (
          <div key={cat} className={catIdx > 0 ? 'border-t border-gray-800' : ''}>
            ...
            <div className="flex items-center gap-2 bg-gray-900/60 px-4 py-2">
              ...
              {categoryCost > 0 && (
                <span className="ml-auto text-xs text-amber-500/60">🪙 {categoryCost.toLocaleString()}</span>
              )}
            </div>
            ...
            <div className="divide-y divide-gray-800/60 bg-gray-950">
              {catRows.map(row => (
                row.category === 'equipment' ? (
                  ...
                  <div
                    key={row.itemId}
                    className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-900/60 transition-colors"
                  >
                    {/* Image / placeholder */}
                    <div
